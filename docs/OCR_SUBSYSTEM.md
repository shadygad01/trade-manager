...
2. **Distribute** — the accumulated pool is grouped by ticker, and each ticker group gets exactly **one** portfolio picker shared by every buy, sell, and verification row for that ticker. This is deliberate: a sell only makes sense against the specific portfolio holding the shares it closes, so assigning a ticker's buys to a portfolio must carry its sells along automatically rather than asking the user to repeat the same choice per row. Manual intervention stops there: once a ticker's portfolio resolves — either it's already unambiguous (an existing ticker recorded in exactly one portfolio, or the only portfolio that exists) or explicitly picked — every pending buy, dividend, and position verification under that ticker commits on its own, with no per-row button. **Sell is the one exception**: which lot(s) a sell closes is an explicit financial decision (ADR-002), so a sell candidate always opens the allocation modal for manual review instead of auto-committing.
...
- **Ticker resolution tolerates OCR letter-garbling**: exact match → prefix match → Levenshtein fuzzy match against `KNOWN_EGX_TICKERS` (`src/domain/value-objects/knownTickers.ts`). A trailing bracketed annotation on the company name (a trading-symbol short-form like "Egyptian International Pharmaceuticals (EIPICO)", or a qualifier like "Commercial International Bank (Egypt)") is stripped before any of these steps run, since OCR renders the bracket glyph inconsistently ("(" vs "{") and, left in place, the same real company could normalize to two different strings and split into two Import ticker groups purely because of which bracket character got OCR'd.
...
These land in the Import page's Step 2 distribution pool alongside buy/sell candidates and verification rows (grouped by ticker, same portfolio picker); once the ticker's portfolio resolves, each auto-commits via `PortfolioService.recordDividend` using the screenshot's own date — so a dividend paid months ago is timelined to that date, not to the moment it was imported. Recorded dividends flow into `equityCurve`/`portfolioReturn` like any other `TimelineEvent`, and are summed across all portfolios in the Dashboard's "Total Dividends" tile.
...
`src/application/services/duplicateDetection.ts` checks each freshly parsed candidate against trades/allocations already on the ledger (same ticker, date, and share count): an **exact** match (price matches too) usually means the same file was re-imported; a **possible** match (price differs) usually means the same real trade was parsed from two different document formats — one commission-inclusive, one not (see the price-from-value note above). These two cases are handled differently now that Buy auto-commits: an **exact** match is auto-skipped — near-certainly the same transaction re-read from a different file, so it's recorded as `skippedKeys` and never becomes a trade — while a **possible** match still auto-commits (a false positive here should never block recording a real trade), but keeps showing its "Possible duplicate" badge afterward so it stays visible for a later look. A sell candidate's duplicate badge is informational only — it never blocks "Allocate Sell", since that action already opens the manual allocation modal.
...
**Low confidence flags, it no longer gates.** A `confidence: "low"` buy/dividend/verification row (`AutoCommitRow` in `ImportPage.tsx`) still auto-commits like any other row — requiring a manual checkbox before every low-confidence row would defeat the point of auto-commit — but renders with a distinct amber color, a warning icon, and its own delete action (`TradeService.deleteTrade`, refunding cost and removing the `Buy` event) so a wrong unmapped-ticker guess is one click to undo after the fact rather than blocked up front. A low-confidence sell (`CandidateRow`) is unaffected either way: it was never gated, since "Allocate Sell" already opens the allocation modal as its own manual review step.

## Correcting a wrong ticker

The header-ticker fallback (used when a screenshot's company name doesn't resolve against `KNOWN_EGX_TICKERS`) only accepts an all-caps token of exactly 4 letters — every real EGX ticker is 4 characters, so a 2-3 letter OCR fragment near the header (a misread label, not the actual ticker) is rejected outright rather than fabricated into a bogus ticker group. This closed one real failure mode, but OCR ticker resolution can never be made perfect for every unmapped company, so `ImportPage`'s Step 2 also lets the ticker itself be corrected directly: clicking a ticker group's heading turns it into an editable field, and confirming a new ticker moves every pending candidate/verification/dividend currently grouped under the old one to the corrected ticker (and carries over its chosen portfolio, if one was already picked). Now that most rows auto-commit almost as soon as they're extracted, a pending-pool-only rename would miss the common case — a row noticed as wrong is usually already a real trade — so renaming also calls `TradeService.renameTickerEverywhere`, which updates the ticker (and re-derives `companyName`/`sector`, clearing the sector if the corrected ticker doesn't resolve to a known one) on every already-recorded `Trade`, `TradeAllocation`, `TimelineEvent`, and `PositionVerification` that still carried the old ticker.
