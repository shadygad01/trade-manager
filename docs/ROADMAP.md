...
5. **Innovative ticker/company-name extraction accuracy improvements** — a separate, explicitly requested follow-up not yet scoped: reduce OCR misreads beyond what `KNOWN_EGX_TICKERS` fuzzy-matching already catches (e.g. using the Invoice format's labeled "Symbol Code" field as a second corroborating signal, or other resolution strategies) now that auto-commit means a wrong ticker guess turns into a real ledger row much faster than before.
