...
3. ~~**OCR confidence-aware UX**~~ — done, then superseded (see above): the original confirmation-checkbox gate was replaced by auto-commit with a visible low-confidence flag + delete action, per direct user feedback that manual intervention should be minimized.
