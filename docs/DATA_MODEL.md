...
- **`Trade.sector`** is optional and set once at buy time, same as `companyName`: `TradeService.recordBuy` auto-assigns it from `src/domain/value-objects/knownSectors.ts`'s ticker→sector map (the same 20-ticker known universe as `knownTickers.ts`, kept as a separate map since a ticker's sector is a distinct fact from its OCR/price-fetch resolution entry), unless the caller passes an explicit `sector` override. A ticker outside that map is left `undefined` — never guessed at — and is folded into an honest "Unclassified" bucket by the `sectorAllocation` calculator (see [ANALYTICS_ENGINE.md](ANALYTICS_ENGINE.md)) rather than fabricating a value to fill the Dashboard's Sector Allocation chart.
