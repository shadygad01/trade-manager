...
- **`Trade`** = one Buy execution. Immutable once created: `ticker`, `shares`, `entryPrice`, `fees`, `executionDate`/`executionTime` are never edited. `remainingShares` starts equal to `shares` and only ever decreases, and only via a `TradeAllocation` being recorded against it — never by direct mutation.
...
- **Cost basis** for the open portion of a trade = `(entryPrice * shares + fees) * (remainingShares / shares)` — a straight pro-rating of the original lot's total cost, not a re-averaged figure blended with other lots.
