...
export function ImportPage() {
  ...
  // One statement/screenshot can hold trades meant for different portfolios
  // (e.g. some shares are "Investment", others are "Trading") — so the
  // portfolio is chosen per candidate/verification row during review, not
  // once for the whole upload.
  const [candidatePortfolio, setCandidatePortfolio] = useState<Record<number, string>>({});
  const [verificationPortfolio, setVerificationPortfolio] = useState<Record<number, string>>({});
  ...
  function portfolioFor(map: Record<number, string>, index: number): string {
    return map[index] ?? portfolios[0]?.id ?? "";
  }
  ...
  async function addBuyCandidate(index: number, candidate: ParsedTradeCandidate) {
    const portfolioId = portfolioFor(candidatePortfolio, index);
  ...
  async function acceptVerification(index: number, verification: ImportResult["verifications"][number]) {
    const portfolioId = portfolioFor(verificationPortfolio, index);
  ...
  return (
    <div>
      <PageHeader
        title="Import"
        description="Drop a broker statement screenshot, PDF, or CSV export — pick which portfolio each operation belongs to, then confirm."
      />

      {portfolios.length === 0 ? (
        <EmptyState
          title="Create a portfolio first"
          description="Import assigns every trade to one of your portfolios, so at least one needs to exist before you can confirm anything here."
          action={
      ...
      <div
        ...
        className={`mt-4 flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-12 text-center transition-colors ${
                    ...
                    {outcome.candidates.map((c, i) => {
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td className="px-4 py-2.5">
                            <select
                              value={portfolioFor(candidatePortfolio, i)}
                              onChange={(e) => setCandidatePortfolio((prev) => ({ ...prev, [i]: e.target.value }))}
                              disabled={addedTickers.has(i)}
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {outcome.verifications.map((v, i) => (
                      <tr key={i}>
                        ...
                        <td className="px-4 py-2.5">
                          <select
                            value={portfolioFor(verificationPortfolio, i)}
                            onChange={(e) => setVerificationPortfolio((prev) => ({ ...prev, [i]: e.target.value }))}
                            disabled={acceptedVerifications.has(i)}
                            className="rounded border border-slate-700 bg-slate-800 px-2 py-1 text-xs text-slate-100 disabled:opacity-50"
                          >
                            {portfolios.map((p) => (
                              <option key={p.id} value={p.id}>
      ...
      >
        {sellCandidate ? (
          <SellAllocationForm
            portfolioId={portfolioFor(candidatePortfolio, sellCandidate.index)}
