...
describe("avgLoser", () => {
  ...
  it("averages only the losing allocations' % return (negative), against each lot's cost basis", () => {
    const trade = makeTrade({ id: "t1", entryPrice: 10, shares: 150 });
    const allocations = [
      makeAllocation({ id: "a1", tradeId: "t1", sharesClosed: 50, exitPrice: 5 }), // -50%
      makeAllocation({ id: "a2", tradeId: "t1", sharesClosed: 50, exitPrice: 2 }), // -80%
      makeAllocation({ id: "a3", tradeId: "t1", sharesClosed: 50, exitPrice: 20 }), // winner, excluded
    ];
    expect(avgLoser(allocations, [trade])).toBeCloseTo((-50 + -80) / 2);
  });
});
